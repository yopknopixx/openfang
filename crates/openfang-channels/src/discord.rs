...
impl DiscordAdapter {
    ...
    async fn send_rich_blocks(
        ...
        // Strategy 1: Single embed (most compatible — works on all platforms including iOS)
        if blocks.len() <= 2 && has_embed && !has_gallery && !has_section {
            for block in blocks {
                match block {
...
-# {}", ft));
        ...
        if v2_ok {
            ...
            self.api_send_message(channel_id, fallback_text).await
        }
    }
