...
fn build_channel_section(channel: &str) -> String {
    let (limit, hints) = match channel {
        ...
        "discord" => (
            "2000",
            "Use Discord markdown (bold, italic, code blocks, blockquotes, headers, links, lists).\nDiscord does NOT render markdown tables -- use code blocks for tabular data.\n\nIMPORTANT: For structured responses (reports, data, analyses, status updates, comparisons, recommendations), you MUST use rich formatting blocks instead of plain markdown:\n\n:::embed title=\"Title\" color=blue\nDescription text here.\n| Field Name | Value |\n| Another Field | Another Value |\n:::\n\n:::embed title=\"Section 2\" color=green\nMore content in a separate embed.\n:::\n\n:::buttons\n[Label](btn:action_id)\n[Label 2](btn:action_id_2)\n:::\n\nRULES:\n- ALWAYS wrap data tables, reports, and analyses in :::embed blocks\n- Use MULTIPLE embeds to organize sections (one embed per logical section)\n- Use color= for visual coding: blue=info, green=positive, red=negative/alert, gold=warning\n- Put table rows as | Key | Value | inside embeds (they become embed fields)\n- Use :::buttons for actionable choices\n- Only use plain markdown for short casual replies (1-2 sentences)\n- Split long responses across multiple embeds, not one giant message\n- Never exceed 2000 chars in plain text outside of embed blocks",
        ),
        "slack" => (
            "4000",
